...
class OvertakingAnalyzer:
    ...
    def analyze(self, frame_w, frame_h, tracked):
        ...
        for v in tracked:
            ...
            if (v.get("direction") == "oncoming"
                    and on_x1 <= cx <= on_x2):
                self._history.append(False)
                return (False,
                        f"Oncoming "
                        f"({v.get('distance',0):.0f}m)")
            if ov_x1 <= cx <= ov_x2:
                self._history.append(False)
                return (False,
                        f"Vehicle in overtake lane "
                        f"({v.get('distance',0):.0f}m)")
...
def run(video_path, cfg):
    ...
    cv2.namedWindow("YOLOPv2 Overtaking Safety",
                    cv2.WINDOW_NORMAL)
    cv2.resizeWindow("YOLOPv2 Overtaking Safety",
        ...
        if not paused:
            ...
            if frame_n % cfg.SKIP_FRAMES == 0:
                ...
                for t in tracks:
                    ...
                    dist = estimator.estimate_distance(
                        x1, y1, x2, y2, vw)
                    spd  = estimator.estimate_speed(
                        tid, dist)
                    dire = estimator.estimate_direction(
                        tid, x1, y1, x2, y2, orig_h)
                ...
                last_tracks = tracked
                ego_spd = estimator.estimate_ego_motion(
                    frame)

                # Safety
                feasible, reason = analyzer.analyze(
                    orig_w, orig_h, tracked)
            ...
            sx = cfg.OUTPUT_WIDTH  / max(orig_w, 1)
            sy = cfg.OUTPUT_HEIGHT / max(orig_h, 1)
            ...
            display = hud.render(
                display, fps, scaled, ego_spd,
                last_safety, last_reason, cfg
            )
            cv2.imshow("YOLOPv2 Overtaking Safety",
                       display)

        # FPS sync
        elapsed = time.time() - loop_start
        wait_ms = max(1, int(
            (frame_delay - elapsed) * 1000))
